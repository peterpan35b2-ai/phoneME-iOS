...
typedef struct _SocketHandle {
  int id;                       /* 32-bit token exposed to MIDP/Java */
  ...
  struct _SocketHandle *next;   /* Next registered socket instance */
  struct _SocketHandle *all_next; /* Next allocated socket instance */
} SocketHandle;
