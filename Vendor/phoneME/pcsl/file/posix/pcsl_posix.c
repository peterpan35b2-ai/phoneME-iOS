...
{
	return close((int)(intptr_t)handle);
}
...
{
	return read((int)(intptr_t)handle, buf, size);
}
...
{
	return write((int)(intptr_t)handle, buffer, size);
}
...
{
	return ftruncate((int)(intptr_t)handle, (off_t)size);
}
...
{
	return lseek((int)(intptr_t)handle, offset, position);
}
...
long pcsl_file_sizeofopenfile(void *handle)
{
    ...
    if ((status = fstat((int)(intptr_t)handle, &stat_buf)) < 0) {
        return status;
    }
...
long pcsl_file_sizeof(const pcsl_string * fileName)
{
    ...
    openStatus = pcsl_file_open(fileName, O_RDONLY, &handle);

    if(openStatus < 0) {
	return -1;
    }
	
     if (fstat((int)(intptr_t)handle, &stat_buf) < 0) {
