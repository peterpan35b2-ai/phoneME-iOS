...
static jfieldID _cgEl_selected_cache     = 0;
static jfieldID _cgEl_font_cache         = 0;

/* cached field ids for Font class */
static jfieldID _f_face_cache  = 0;
static jfieldID _f_style_cache = 0;
static jfieldID _f_size_cache  = 0;

#define _CACHE_FIELDID(HANDLE, name, type, cache) \
     ((cache)==0?((cache)=KNI_GetFieldID((HANDLE), (name), (type))):(cache))
/**
...
Java_javax_microedition_lcdui_ChoiceGroupLFImpl_createNativeResource0() {
  ...
  KNI_ReturnInt(cgPtr == NULL ? 0 : MidpComponentToId(&cgPtr->component));
}
...
Java_javax_microedition_lcdui_ChoiceGroupLFImpl_insert0() {
  MidpError err = KNI_OK;
  MidpItem *cgPtr = MidpItemFromId(KNI_GetParameterAsInt(1));
