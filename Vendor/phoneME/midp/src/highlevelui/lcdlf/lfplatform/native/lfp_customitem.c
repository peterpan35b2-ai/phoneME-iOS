...
Java_javax_microedition_lcdui_CustomItemLFImpl_createNativeResource0() {
    ...
    MidpDisplayable *ownerPtr = MidpDisplayableFromId(
...
cleanup:
    ...
    KNI_ReturnInt(ciPtr == NULL ? 0 : MidpComponentToId(&ciPtr->component));
}
...
Java_javax_microedition_lcdui_CustomItemLFImpl_refresh0() {
    ...
    MidpItem *ciPtr = MidpItemFromId(KNI_GetParameterAsInt(1));
    jint x = KNI_GetParameterAsInt(3);
    jint y = KNI_GetParameterAsInt(4);
    jint width = KNI_GetParameterAsInt(5);
    jint height = KNI_GetParameterAsInt(6);
    MidpError err;
    ...
    err = lfpport_customitem_refresh(ciPtr, x, y, width, height);
    KNI_EndHandles();
