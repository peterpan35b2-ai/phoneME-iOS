...
static jboolean checkForSocketPointerAndKeyboardSignal(MidpReentryData* pNewSignal,
    ...
    if (phoneme_sdl3_has_pending_key()) {
        handleKey(pNewSignal, pNewMidpEvent);
        return KNI_TRUE;
    }
    if (phoneme_sdl3_has_pending_pointer()) {
    ...
    if (phoneme_ios_port_has_pending_pointer()) {
        handlePointer(pNewSignal, pNewMidpEvent);
        return KNI_TRUE;
    }
    if (timeout64 < 0 || timeout64 > 16) {
...
#if defined(PHONEME_SDL3) || defined(PHONEME_IOS_NATIVE)
    else if (timeout64 > 0) {
        usleep((useconds_t)(timeout64 * 1000));
    }
#endif
