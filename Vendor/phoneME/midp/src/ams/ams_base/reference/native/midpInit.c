...
#if defined(DARWIN)
            /*
             * The legacy phone profile limits all installed suites and RMS
             * data to 1 MB through system.jam_space. A desktop emulator can
             * easily exceed that just by installing one modern JAR, causing
             * every RecordStore create to fail with RecordStoreFullException.
             * Use the largest value exposed by the Java int-based RMS API.
             */
            storageSetTotalSpace(INT_MAX);
#else
