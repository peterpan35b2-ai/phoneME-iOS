...
public class FileInstaller extends Installer {
    ...
    /**
     * Converts a file URL into a native file-system path.
     *
     * Installer.getUrlPath() removes the leading slash from absolute paths
     * for compatibility with Windows drive-letter paths. Restore it for
     * Unix-style paths while leaving paths such as C:/games/foo.jar intact.
     *
     * @param url file URL or local path
     * @return decoded native file-system path
     */
    private static String getLocalFileName(String url) {
        String path = getUrlPath(FileUrl.encodeFilePath(url));
        ...
        return FileUrl.decodeFilePath(path);
    }
    ...
    protected byte[] downloadJAD() throws IOException {
        if (info.jadUrl.endsWith(".jar")) {
            ...
            String jadFilename = getLocalFileName(info.jadUrl);
    ...
    protected int downloadJAR(String filename) throws IOException {
        ...
        String jarFilename = getLocalFileName(info.jarUrl);
               
        // Open source (jar) file
        try {
            conn = new RandomAccessStream();
            ((RandomAccessStream)conn).connect(
                    jarFilename, Connector.READ);
