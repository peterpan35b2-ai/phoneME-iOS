...
char* getMidpHome(char *cmd, char *dirBuffer) {
    ...
    /*
     * If MIDP_HOME is set, copy it into the caller-provided buffer. The old
     * code returned the environment pointer, but callers intentionally use
     * dirBuffer and ignored that return value, producing "appdb" relative to
    ...
    if (midp_home != NULL) {
        ...
        if (length > 0 && dirBuffer[length - 1] != *filesep &&
                length < MAX_FILENAME_LENGTH) {
            dirBuffer[length] = *filesep;
            dirBuffer[length + 1] = 0;
        }
        return dirBuffer;
    }
