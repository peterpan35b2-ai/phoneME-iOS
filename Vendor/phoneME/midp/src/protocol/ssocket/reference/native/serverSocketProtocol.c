...
static jfieldID serverSocketNativeHandleField;
static jfieldID clientSocketHandleField;
static int serverSocketFieldsInitialized;
static int clientSocketFieldsInitialized;

#define SERVER_SOCKET_GET_HANDLE(object) \
    KNI_GetIntField((object), serverSocketNativeHandleField)
#define SERVER_SOCKET_SET_HANDLE(object, value) \
    KNI_SetIntField((object), serverSocketNativeHandleField, \
                    (jint)(intptr_t)(value))
#define CLIENT_SOCKET_SET_HANDLE(object, value) \
    KNI_SetIntField((object), clientSocketHandleField, \
                    (jint)(intptr_t)(value))
#define SERVER_SOCKET_HANDLE(value) ((void*)(intptr_t)(jint)(value))
...
Java_com_sun_midp_io_j2me_serversocket_Socket_open0(void) {
    ...
    if (!serverSocketFieldsInitialized) {
        KNI_GetObjectClass(thisObject, classObject);
        serverSocketNativeHandleField =
            KNI_GetFieldID(classObject, "nativeHandle", "I");
        serverSocketFieldsInitialized = 1;
    }
    port = (int)KNI_GetParameterAsInt(1);
    suiteId = KNI_GetParameterAsInt(2);

    SERVER_SOCKET_SET_HANDLE(thisObject, INVALID_HANDLE);
    ...
    if (suiteId == UNUSED_SUITE_ID) {
        ...
        if (pushReturn == -1) {
            ...
            SERVER_SOCKET_SET_HANDLE(thisObject, pushReturn);
            tryOpen = KNI_FALSE;
        }
    }
...
Java_com_sun_midp_io_j2me_serversocket_Socket_accept0(void) {
    ...
    KNI_DeclareHandle(socketClassObject);

    KNI_GetThisPointer(thisObject);
    KNI_GetParameterAsObject(1, socketObject);
    if (!clientSocketFieldsInitialized) {
        KNI_GetObjectClass(socketObject, socketClassObject);
        clientSocketHandleField =
            KNI_GetFieldID(socketClassObject, "handle", "I");
        clientSocketFieldsInitialized = 1;
    }
    ...
    if (serverSocketHandle == (int)(intptr_t)INVALID_HANDLE) {
        ...
        if (connectionHandle != (void *)(intptr_t)-1) {
             ...
             * platform.
             */
            CLIENT_SOCKET_SET_HANDLE(socketObject, connectionHandle);
        }
    }
