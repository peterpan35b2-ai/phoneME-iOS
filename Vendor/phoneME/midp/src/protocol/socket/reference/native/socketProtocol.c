...
static jfieldID socketHandleField;
static jfieldID socketPendingConnectionsField;
static jfieldID socketIStreamsField;
static jfieldID socketOStreamsField;
static int socketFieldsInitialized;

#define PCSL_TOKEN(handle) ((jint)(intptr_t)(handle))
#define SOCKET_GET_HANDLE(object) \
    KNI_GetIntField((object), socketHandleField)
#define SOCKET_SET_HANDLE(object, value) \
    KNI_SetIntField((object), socketHandleField, PCSL_TOKEN(value))
#define SOCKET_GET_PENDING(object) \
    KNI_GetIntField((object), socketPendingConnectionsField)
#define SOCKET_SET_PENDING(object, value) \
    KNI_SetIntField((object), socketPendingConnectionsField, (jint)(value))

#define SOCK_ANC_INC_NETWORK_INDICATOR \
    ANC_INC_NETWORK_INDICATOR \
    SOCKET_SET_PENDING(thisObject, SOCKET_GET_PENDING(thisObject) + 1);
...
/*
 * Do not access Java fields through ROMStructs on LP64. The VM stores Java
 * references in its own layout, which is not guaranteed to match a native C
 * structure containing 64-bit pointers. KNI field accessors use the VM's real
 * offsets and avoid overwriting adjacent fields such as BufferedConnectionAdapter.eof.
 */
...
Java_com_sun_midp_io_j2me_socket_Protocol_open0(void) {
    ...
    KNI_DeclareHandle(classObject);
    KNI_GetThisPointer(thisObject);
    KNI_GetParameterAsObject(1, bufferObject);

    if (!socketFieldsInitialized) {
        KNI_GetObjectClass(thisObject, classObject);
        socketHandleField = KNI_GetFieldID(classObject, "handle", "I");
        socketPendingConnectionsField =
            KNI_GetFieldID(classObject, "pendingConnections", "I");
        socketIStreamsField = KNI_GetFieldID(classObject, "iStreams", "I");
        socketOStreamsField = KNI_GetFieldID(classObject, "oStreams", "I");
        socketFieldsInitialized = 1;
    }

    info = (MidpReentryData*)SNI_GetReentryData(NULL);
    if (info == NULL) {   /* First invocation */
       SOCKET_SET_HANDLE(thisObject, INVALID_HANDLE);
       ...
       /**
        ...
        if (midpCheckResourceLimit(RSC_TYPE_TCP_CLI, 1) == 0) {
            ...
            if (status == PCSL_NET_SUCCESS) {
                SOCKET_SET_HANDLE(thisObject, pcslHandle);
                ...
                REPORT_INFO1(LC_PROTOCOL, " handle = %d\n",
                             PCSL_TOKEN(pcslHandle));
                midp_thread_wait(NETWORK_WRITE_SIGNAL, PCSL_TOKEN(pcslHandle),
                                 context);
            } else {
        ...
        pcslHandle = (void *)(intptr_t)info->descriptor;
        /* midp_thread_wait stores the platform continuation in pResult;
         * status is reserved for the signal result. Mixing them breaks
         * asynchronous socket connect implementations. */
        context = info->pResult;

        if (SOCKET_GET_HANDLE(thisObject) != PCSL_TOKEN(pcslHandle)) {
            REPORT_CRIT2(LC_PROTOCOL,
                         "socket::open Handles mismatched 0x%x != 0x%x\n",
                         pcslHandle,
                         SOCKET_GET_HANDLE(thisObject));
        }
...
Java_com_sun_midp_io_j2me_socket_Protocol_read0(void) {
    ...
    pcslHandle = (void *)(intptr_t)SOCKET_GET_HANDLE(thisObject);
    iStreams = (int)KNI_GetIntField(thisObject, socketIStreamsField);

    REPORT_INFO3(LC_PROTOCOL, "socket::read0 o=%d l=%d fd=%d\n",
                 offset, length, PCSL_TOKEN(pcslHandle));
    ...
    if (pcslHandle != INVALID_HANDLE) {
        ...
        SNI_BEGIN_RAW_POINTERS;
        bytesRead = pushgetcachedpacket(PCSL_TOKEN(pcslHandle),
            &ipAddress, &port,
    ...
    if (bytesRead <= 0) {
        ...
        if (info == NULL) {   /* First invocation */
            ...
            if (INVALID_HANDLE == pcslHandle || iStreams == 0) {
                ...
                if ((void *)(intptr_t)info->descriptor != pcslHandle) {
                    REPORT_CRIT2(LC_PROTOCOL,
...
Java_com_sun_midp_io_j2me_socket_Protocol_write0(void) { 
    ...
    pcslHandle = (void *)(intptr_t)SOCKET_GET_HANDLE(thisObject);
    oStreams = (int)KNI_GetIntField(thisObject, socketOStreamsField);
...
Java_com_sun_midp_io_j2me_socket_Protocol_available0(void) {
    ...
    if (INVALID_HANDLE == pcslHandle) {
        ...
        /* Check the push cache for a waiting packet. */
        bytesAvailable = pushcacheddatasize(PCSL_TOKEN(pcslHandle));
        if (bytesAvailable <= 0) {
...
Java_com_sun_midp_io_j2me_socket_Protocol_close0(void) {
    ...
    if (info == NULL) {
        ...
        if (INVALID_HANDLE == pcslHandle) {
            ...
            midp_thread_signal(NETWORK_READ_SIGNAL, PCSL_TOKEN(pcslHandle), 0);
            midp_thread_signal(NETWORK_WRITE_SIGNAL, PCSL_TOKEN(pcslHandle), 0);
        }
    } else {
        /* reinvocation */
        pcslHandle = (void *)(intptr_t)info->descriptor;
...
Java_com_sun_midp_io_j2me_socket_Protocol_finalize(void) {
    ...
    i = SOCKET_GET_PENDING(thisObject);
    while (i > 0) {
        ANC_DEC_NETWORK_INDICATOR;
        i--;
    }
...
Java_com_sun_midp_io_j2me_socket_Protocol_getIpNumber0(void) {
    ...
    if (status == PCSL_NET_SUCCESS) {
        ...
        midp_thread_wait(HOST_NAME_LOOKUP_SIGNAL,
                         (int)(intptr_t)pcslHandle, context);
