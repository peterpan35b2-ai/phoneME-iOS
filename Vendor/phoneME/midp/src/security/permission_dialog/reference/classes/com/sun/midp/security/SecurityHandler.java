...
public final class SecurityHandler {
    ...
    /**
     * Desktop emulator policy: installed MIDlets run with all supported
     * permissions and never display interactive permission prompts.
     */
    private static final boolean ALLOW_ALL_PERMISSIONS = true;
    ...
    private void init(byte[] apiPermissions, String domain) {
        ...
        if (ALLOW_ALL_PERMISSIONS) {
            permissions = new byte[maxPermissionLevels.length];
            for (int i = 0; i < permissions.length; i++) {
                permissions[i] = Permissions.ALLOW;
                maxPermissionLevels[i] = Permissions.ALLOW;
            }
            trusted = true;
        } else {
