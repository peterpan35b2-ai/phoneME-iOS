...
rmsdb_get_record_store_space_available(int handle, int pStorageId) {
     ...
     */
    /*
     * Java exposes this value as a signed 32-bit int. On LP64 platforms,
     * LONG_MAX is much larger than Integer.MAX_VALUE, so clamping to
     * LONG_MAX and then casting to jint wraps ordinary free-space values
     * above 2 GiB into a negative number. RecordStoreImpl interprets that
     * as no space and throws RecordStoreFullException.
     */
    availSpaceUpTo2Gb =
        (availSpace <= (jlong)INT_MAX) ? (long)availSpace : (long)INT_MAX;
