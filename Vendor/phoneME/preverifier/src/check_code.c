...
verify_opcode_operands(context_type *context, int inumber, int offset)
{
    ...
    uint32_t *lpc = (uint32_t *) UCALIGN(code + offset + 1);
    uint32_t *lptr;
    int *saved_operand;
    int keys;
    int k, delta;
    if (opcode == opc_tableswitch) {
        ...
        int32_t this_key = (int32_t)ntohl(lptr[0]);
        int32_t next_key = (int32_t)ntohl(lptr[2]);
...
static int instruction_length(unsigned char *iptr)
{
    ...
    switch (instruction) {
        case opc_tableswitch: {
        uint32_t *lpc = (uint32_t *) UCALIGN(iptr + 1);
