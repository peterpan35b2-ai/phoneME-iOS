...
# Desktop MIDlets are loaded dynamically and may call protected CLDC methods
# that no ROMized system class references (for example Object.clone on arrays).
# Apply this after EnableAllROMOptimizations so the aggregate option cannot
# turn dead-method removal back on.
ifeq ($(target_os),darwin)
ROM_GEN_ARG         += -RemoveDeadMethods
endif
...
EXTRA_JVMCONFIG_linux   = USE_UNICODE_FOR_FILENAMES=1
else
...
# The legacy i386 backend requires a 32-bit link. Portable C and
# non-i386 targets retain the selected toolchain architecture.
ifeq ($(gcc_arch), i386)
LINK_FLAGS             += -m32
endif
...
# The legacy i386 stubs require 32-bit assembly.
