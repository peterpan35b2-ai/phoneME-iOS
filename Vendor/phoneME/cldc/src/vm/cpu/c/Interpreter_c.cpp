...
#define WIDE_OFFSET       255
  ...
  typedef address_word native_arg_t;

  // Darwin/arm64 has a distinct ABI for variadic arguments. Runtime helpers
  // are fixed-signature functions, so calling them through a (...) function
  // pointer puts their arguments in the wrong registers. Dispatch through
  // fixed-arity function pointer types instead.
#ifndef PRODUCT
#define CALL_NATIVE_0(result_type, entry) \
  ((result_type (*)(Thread*, Traps*))(entry))(NATIVE_ARG, NULL)
#define CALL_NATIVE_1(result_type, entry, arg1) \
  ((result_type (*)(Thread*, native_arg_t, Traps*))(entry)) \
    (NATIVE_ARG, (arg1), NULL)
#define CALL_NATIVE_2(result_type, entry, arg1, arg2) \
  ((result_type (*)(Thread*, native_arg_t, native_arg_t, Traps*))(entry)) \
    (NATIVE_ARG, (arg1), (arg2), NULL)
#else
#define CALL_NATIVE_0(result_type, entry) \
  ((result_type (*)(Thread*))(entry))(NATIVE_ARG)
#define CALL_NATIVE_1(result_type, entry, arg1) \
  ((result_type (*)(Thread*, native_arg_t))(entry))(NATIVE_ARG, (arg1))
#define CALL_NATIVE_2(result_type, entry, arg1, arg2) \
...
#define ADVANCE_FOR_RETURN() \
  ADVANCE((int)(intptr_t)GET_FRAME(return_advance))
...
#define BOUNDS_CHECK(obj, idx) \
  ...
  // Method locals are Java slots. On LP64 each slot is pointer-sized even
  // when it currently contains a 32-bit primitive.
#define LOCAL_SLOT(n) (g_jlocals - (n) * BytesPerStackElement)
#define GET_LOCAL(n) (*(jint*)LOCAL_SLOT(n))
#define GET_LOCAL_ADDR(n) (*(address*)LOCAL_SLOT(n))
#define GET_LOCAL_LOW(n) (*(jint*)LOCAL_SLOT(n))
#define GET_LOCAL_HIGH(n) (*(jint*)LOCAL_SLOT((n) + 1))

#define SET_LOCAL(n, val) (*(jint*)LOCAL_SLOT(n) = (val))
#define SET_LOCAL_ADDR(n, val) (*(address*)LOCAL_SLOT(n) = (val))
#define SET_LOCAL_LOW(n, val) (*(jint*)LOCAL_SLOT(n) = (val))
#define SET_LOCAL_HIGH(n, val) (*(jint*)LOCAL_SLOT((n) + 1) = (val))
...
#define SET_ENTRY_FRAME(name, value) \
        *(address_word*)(g_jfp + ENTRY_FRAME_OFFSET(name)) = \
          (address_word)(value)
...
#define SET_THREAD_INT(name, value) \
       *(jint*)((address)_current_thread + THREAD_OFFSET(name)) = (jint)(value)
#define GET_THREAD_ADDR(name) \
       *(address*)((address)_current_thread + THREAD_OFFSET(name))
#define SET_THREAD_ADDR(name, value) \
       *(address*)((address)_current_thread + THREAD_OFFSET(name)) = \
         (address)(value)

  // entry access
#define ENTRY_OFFSET(name) EntryActivation::name##_offset()
#define GET_ENTRY_INT(entry, name) \
       *(jint*)((address)entry + ENTRY_OFFSET(name))
#define GET_ENTRY_ADDR(entry, name) \
       *(address*)((address)entry + ENTRY_OFFSET(name))
...
#define BREAK_INTERPRETER_LOOP(reason)                                \
  ...
  static inline jushort first_ushort_from_cpool(address cpool, int index) {
    ...
    cpool += index * BytesPerWord;
    return (*cpool | ((jushort)*(cpool + 1)) << 8);
#else
    cpool += index * BytesPerWord + 2;
  ...
  static inline address get_method_from_ci(address ci, jint idx) {
    return *(address*)(ci + idx * BytesPerWord +
                       ClassInfoDesc::header_size());
  }
  ...
  static inline address_word get_from_cpool(address cpool, jushort index) {
    return *(address_word*)(cpool + index * BytesPerWord);
  }
  ...
  // Stack accessors. Every logical Java slot has BytesPerStackElement bytes;
  // primitive values occupy the low bytes while object references use the
  // complete native-width slot.
  static inline address_word SLOT_POP() {
    const address_word value = *(address_word*)g_jsp;
    g_jsp += BytesPerStackElement;
    return value;
  }

  static inline void SLOT_PUSH(address_word value) {
    g_jsp -= BytesPerStackElement;
    *(address_word*)g_jsp = value;
  }

  static inline address_word SLOT_PEEK(jint n) {
    return *(address_word*)(g_jsp + n * BytesPerStackElement);
  }

  static inline jint POP() {
    const jint value = *(jint*)g_jsp;
    g_jsp += BytesPerStackElement;
    return value;
  }

  static inline address OBJ_POP() {
    const address value = *(address*)g_jsp;
    g_jsp += BytesPerStackElement;
    return value;
  }

  static inline jfloat FLOAT_POP() {
    const jfloat value = *(jfloat*)g_jsp;
    g_jsp += BytesPerStackElement;
    return value;
  }
  ...
  static inline jint PEEK(jint n) {
    return *(jint*)(g_jsp + n * BytesPerStackElement);
  }

  static inline address OBJ_PEEK(jint n) {
    return *(address*)(g_jsp + n * BytesPerStackElement);
  }
  ...
  static inline address callee_method() {
    return GET_THREAD_ADDR(obj_value);
  }

  static inline void set_callee_method(address method) {
    SET_THREAD_ADDR(obj_value, method);
  ...
  static bool lock_object(address object, address lock) {
    ...
    *(address*)(lock + StackLock::size()) = object;
    ...
    if ((jint)(intptr_t)GET_NEAR(near_obj, raw_value) & 0x1) {
      // maybe slow case
    ...
    *(address*)object = lock + StackLock::copied_near_offset();
    ...
    SET_STACKLOCK_OFFSET(lock, copied_near, 0,
                         *(address*)(near_obj + 0));
    SET_STACKLOCK_OFFSET(lock, copied_near, BytesPerWord,
                         *(address*)(near_obj + BytesPerWord));
    // set lock bit
    const jint raw_value =
      *(jint*)(near_obj + 2 * BytesPerWord) | 0x1;
    SET_STACKLOCK_OFFSET(lock, copied_near, 2 * BytesPerWord,
                         (address)(intptr_t)raw_value);
    return false;
  }
  ...
  static address allocate_monitor() {
    ...
    const jint len = StackLock::size() + BytesPerWord;
    g_jsp -= len;
    lock -= len;
    ...
    address tmp1 = g_jsp;
    while (tmp1 != lock) {
      *(address_word*)tmp1 = *(address_word*)(tmp1 + len);
      tmp1 += BytesPerWord;
    }
  ...
  static void invoke_native_entry(address entry_point,
                                  ...
                                  jint num_args,
                                  va_list ap,
                                  native_arg_t &r1, jint &r2)
  {
    native_arg_t arg1 = 0;
    native_arg_t arg2 = 0;
    if (num_args >= 1) {
      arg1 = va_arg(ap, native_arg_t);
    }
    if (num_args >= 2) {
      arg2 = va_arg(ap, native_arg_t);
    }

    switch (return_value_type) {
    case T_INT:
      switch (num_args) {
      case 0: r1 = CALL_NATIVE_0(jint, entry_point); break;
      case 1: r1 = CALL_NATIVE_1(jint, entry_point, arg1); break;
      case 2: r1 = CALL_NATIVE_2(jint, entry_point, arg1, arg2); break;
    ...
    case T_LONG:
      {
        jlong_accessor acc;
        switch (num_args) {
        case 0: acc.long_value = CALL_NATIVE_0(jlong, entry_point); break;
        case 1: acc.long_value = CALL_NATIVE_1(jlong, entry_point, arg1); break;
        case 2: acc.long_value = CALL_NATIVE_2(jlong, entry_point, arg1, arg2); break;
        default: SHOULD_NOT_REACH_HERE(); return;
        }
        r1 = acc.words[0];
        r2 = acc.words[1];
      }
    ...
    case T_FLOAT:
      {
        jfloat value;
        switch (num_args) {
        case 0: value = CALL_NATIVE_0(jfloat, entry_point); break;
        case 1: value = CALL_NATIVE_1(jfloat, entry_point, arg1); break;
        case 2: value = CALL_NATIVE_2(jfloat, entry_point, arg1, arg2); break;
        default: SHOULD_NOT_REACH_HERE(); return;
        }
        r1 = float_bits(value);
      }
    ...
    case T_DOUBLE:
      {
        jdouble_accessor acc;
        switch (num_args) {
        case 0: acc.double_value = CALL_NATIVE_0(jdouble, entry_point); break;
        case 1: acc.double_value = CALL_NATIVE_1(jdouble, entry_point, arg1); break;
        case 2: acc.double_value = CALL_NATIVE_2(jdouble, entry_point, arg1, arg2); break;
        default: SHOULD_NOT_REACH_HERE(); return;
        }
        r1 = acc.words[0];
        r2 = acc.words[1];
    ...
    case T_ARRAY:
      switch (num_args) {
      case 0:
        r1 = (native_arg_t)CALL_NATIVE_0(OopDesc*, entry_point);
        break;
      case 1:
        r1 = (native_arg_t)CALL_NATIVE_1(OopDesc*, entry_point, arg1);
        break;
      case 2:
        r1 = (native_arg_t)CALL_NATIVE_2(OopDesc*, entry_point, arg1, arg2);
        break;
      default:
    ...
    case T_VOID:
      switch (num_args) {
      case 0: CALL_NATIVE_0(void, entry_point); break;
      case 1: CALL_NATIVE_1(void, entry_point, arg1); break;
      case 2: CALL_NATIVE_2(void, entry_point, arg1, arg2); break;
      default: SHOULD_NOT_REACH_HERE();
      }
      break;
  ...
  static bool resume_thread() {
    ...
    g_jsp = GET_THREAD_ADDR(stack_pointer);
    g_jfp = OBJ_POP();
    // remove dummy
    POP();
    ...
    address pentry = GET_THREAD_ADDR(pending_entries);
    if (pentry) {
      if (GET_THREAD_INT(status) & THREAD_TERMINATING) {
        SET_THREAD_ADDR(pending_entries, NULL);
        BREAK_INTERPRETER_LOOP(JMP_THREAD_EXIT);
      } else {
        jint max_len = GET_ENTRY_INT(pentry, length);
        while ((pentry = GET_ENTRY_ADDR(pentry, next)) != NULL) {
          const jint len = GET_ENTRY_INT(pentry, length);
          if (len > max_len) {
            max_len = len;
          }
        }
        max_len = max_len * 2 * BytesPerStackElement +
                  StackLock::size() + BytesPerWord;
        // stack grows to smaller addresses
        address new_stack_ptr = GET_THREAD_ADDR(stack_pointer) - max_len;
        // increase stack size
        if (new_stack_ptr < _current_stack_limit) {
          if (interpreter_call_vm_1((address)&stack_overflow, T_VOID,
                                    (native_arg_t)new_stack_ptr)) {
            return true;
          }
        }
    ...
    SET_THREAD_ADDR(last_java_sp, NULL);
    SET_THREAD_ADDR(last_java_fp, NULL);

    if (CURRENT_HAS_PENDING_EXCEPTION) {
      OopDesc* exception =_current_pending_exception;
      _current_pending_exception = NULL;
      shared_call_vm_internal(NULL, NULL, T_ILLEGAL, 1,
                              (native_arg_t)exception);
      return true;
    }
    return false;
  }
  ...
  static bool shared_call_vm_internal(address entry_point,
                                      ...
                                      ...)
  {
    // Save last Java stack/frame pointers at native width.
    SET_THREAD_ADDR(last_java_fp, g_jfp);
    SET_THREAD_ADDR(last_java_sp, g_jsp);
    ...
    // fp
    OBJ_PUSH(g_jfp);
    SET_THREAD_ADDR(stack_pointer, g_jsp);

    const bool is_oop = (stack_type_for(return_value_type) == T_OBJECT);
    ...
    if (return_value_type == T_ILLEGAL) {
      ...
      OopDesc* raw_exc = (OopDesc*)va_arg(ap, native_arg_t);

      // This call also fills in last_java_fp and last_java_sp in the
      // current thread to the frame of the handler.
      r1 = (native_arg_t)find_exception_frame(NATIVE_ARG, raw_exc);

      g_jsp = GET_THREAD_ADDR(last_java_sp);
      g_jfp = GET_THREAD_ADDR(last_java_fp);
  ...
  static void shared_entry(address return_point) {
    ...
    SET_ENTRY_FRAME(pending_exception,   _current_pending_exception);
    SET_ENTRY_FRAME(stored_last_sp,      GET_THREAD_ADDR(last_java_sp));
    SET_ENTRY_FRAME(stored_last_fp,      GET_THREAD_ADDR(last_java_fp));
    SET_ENTRY_FRAME(fake_return_address, EntryFrame::FakeReturnAddress);
    SET_ENTRY_FRAME(real_return_address, return_point);
    SET_ENTRY_FRAME(stored_obj_value,    GET_THREAD_ADDR(obj_value));
    ...
    address pending_entry = GET_THREAD_ADDR(pending_entries);
    ...
    address entry = GET_ENTRY_ADDR(pending_entry, next);
    const jint len = GET_ENTRY_INT(pending_entry, length);
    ...
    // Put complete native-width value slots on the Java stack, skipping tags.
    for (int i = 0; i < len; i++) {
      SLOT_PUSH(*(address_word*)(header + 2 * i * BytesPerWord));
    }

    // find Java method
    address method = GET_ENTRY_ADDR(pending_entry, method);
    // invoke it
  ...
  bool interpreter_grow_stack_internal(address method,
    ...
    g_jpc = DERIVED(address, method,
                     Method::base_offset() + JavaFrame::overflow_frame_flag);
  ...
  static void quick_native_method_entry(BasicType return_type) {
    ...
    if (_jvm_quick_native_exception != NULL) {
      interpreter_call_vm_2((address)&quick_native_throw, T_INT,
        (native_arg_t)_jvm_quick_native_exception, 0);
  ...
  static inline bool get_method_task_mirror(address method,
    ...
    task_mirror = GET_THREAD_ADDR(obj_value);
  ...
  static inline bool is_initialized_class(address klass) {
    ...
    if (status & JavaClassObj::IN_PROGRESS) {
      address this_thread = GET_THREAD_ADDR(thread_obj);
  ...
  void interpreter_method_entry() {
    ...
    address new_stack_ptr =
      g_jsp - get_max_stack(method) * BytesPerStackElement;
  ...
  static BasicType get_fast_accessor_type() {
    address method = callee_method();
    return (BasicType)*(jushort*)(method + Method::fast_accessor_type_offset());
  }
  ...
  void shared_fast_getint_accessor() {
    address obj = OBJ_POP();
    address field = obj + get_fast_accessor_offset();
    BasicType type = get_fast_accessor_type();
    if (type == T_OBJECT || type == T_ARRAY) {
      OBJ_PUSH(*(address*)field);
    } else {
      PUSH(*(jint*)field);
    }
    ADVANCE_FOR_RETURN();
  ...
  static bool array_load(BasicType type) {
    ...
    case T_OBJECT  :
      {
        address v = GET_ARRAY_ELEMENT(ref, idx, address);
        OBJ_PUSH(v);
      }
      break;
    default        :
    ...
    case T_OBJECT  :
      {
        ...
        write_barrier(ref + Array::base_offset() + idx * BytesPerWord);
  ...
  static address get_static_field_offset() {
    ...
    if (obj == get_cib_marker()) {
      ...
      obj = GET_THREAD_ADDR(obj_value);
      cpool = GET_FRAME(cpool);
    }
#else
    address obj = get_class_by_id(class_id);
    if (!is_initialized_class(obj)) {
      if (interpreter_call_vm_1((address)initialize_class, T_VOID,
                                (native_arg_t)obj)) {
        return NULL;
      }
  ...
  static void return_internal(BasicType type) {
    ...
    switch(type) {
    case T_OBJECT :
      rv.object_val = address_from_sp(0);
      break;
    case T_INT    :
    case T_FLOAT  :
    ...
    if (GET_FRAME(return_address) == (address)EntryFrame::FakeReturnAddress) {
      ...
      if (pending_entry != NULL) {
        ...
        address header = pending_entry + EntryActivationDesc::header_size();
        // Put complete native-width values on stack, skipping tags.
        for (int i = 0; i < len; i++) {
          SLOT_PUSH(*(address_word*)(header + 2 * i * BytesPerWord));
        }
        // find Java method
      ...
      SET_THREAD_ADDR(obj_value, GET_ENTRY_FRAME(stored_obj_value));
      // IMPL_NOTE: write_barrier((address)_current_thread + Thread::obj_value_offset());
      SET_THREAD_INT(int1_value,
                     (jint)(intptr_t)GET_ENTRY_FRAME(stored_int_value1));
      SET_THREAD_INT(int2_value,
                     (jint)(intptr_t)GET_ENTRY_FRAME(stored_int_value2));
      ...
      g_jfp = old_jfp;
      SET_THREAD_ADDR(last_java_sp, old_jsp);
      SET_THREAD_ADDR(last_java_fp, old_jfp);
    ...
    g_jpc = GET_FRAME(bcp_store) +
            (int)(intptr_t)GET_FRAME(return_advance);
    g_jlocals = GET_FRAME(locals_pointer);
  ...
  static void fast_ldc(BasicType type, bool wide) {
    ...
    case T_INT:
    case T_FLOAT:
      // fast_1_ldc is shared by ints, floats and object constants. Copy the
      // complete CP slot so a resolved String oop is not truncated on LP64;
      // primitive consumers still read only the low 32 bits.
      SLOT_PUSH(get_from_cpool(cpool, idx));
      break;
    case T_OBJECT:
      OBJ_PUSH((address)get_from_cpool(cpool, idx));
      break;
    case T_LONG   :
      {
        jlong val = long_from_addr(cpool + idx * BytesPerWord);
        LONG_PUSH(val);
      }
      break;
    case T_DOUBLE:
      {
        jdouble val = double_from_addr(cpool + idx * BytesPerWord);
        DOUBLE_PUSH(val);
      }
      break;
    default       :
...
#define BYTECODE_IMPL_END }
  ...
  BYTECODE_IMPL(dup)
    address_word v = SLOT_PEEK(0);
    SLOT_PUSH(v);
    ADVANCE(1);
  BYTECODE_IMPL_END

  BYTECODE_IMPL(dup_x1)
    address_word val1 = SLOT_POP();
    address_word val2 = SLOT_POP();
    SLOT_PUSH(val1);
    SLOT_PUSH(val2);
    SLOT_PUSH(val1);
    ADVANCE(1);
  ...
  BYTECODE_IMPL(if_acmpeq)
    address val2 = OBJ_POP();
    address val1 = OBJ_POP();
    branch(val1 == val2);
  BYTECODE_IMPL_END
  ...
  BYTECODE_IMPL(tableswitch)
    ...
    address aligned_jpc =
      (address)(((uintptr_t)(g_jpc + 1 + 3)) & ~(uintptr_t)3);
    // get default target
  ...
  static void new_return_point() {
    OBJ_PUSH(GET_THREAD_ADDR(obj_value));
    ADVANCE(3);
  }
  ...
  BYTECODE_IMPL(fast_1_getstatic)
    address addr = get_static_field_offset();
    if (addr != NULL) {
      // This bytecode is shared by one-word primitives and oop statics.
      // Preserve the complete LP64 slot; primitive users consume the low word.
      SLOT_PUSH(*(address_word*)addr);
      ADVANCE(3);
    }
  BYTECODE_IMPL_END
  ...
  BYTECODE_IMPL(fast_invokeinterface)
    ...
    // start of itable
    address itable = ci + ClassInfoDesc::header_size()
                    + vlength * sizeof(jobject);

    // Lookup interface method table by linear search
    for (itable = ci + ClassInfoDesc::header_size()
                   + vlength * sizeof(jobject); ; ilength--){
      // IMPL_NOTE: or < 0
      if (ilength <= 0) {
    ...
    address method = *(address*)(table + method_index * sizeof(jobject));
  ...
  static void invokenative_return_point() {
    ...
    // Clear async_info (even if the method was never redone).
    SET_THREAD_ADDR(async_info, NULL);
  ...
  BYTECODE_IMPL(fast_invokenative)
    ...
    // Set space for fake parameter for static method (KNI-ism).
    // A Java parameter slot is pointer-sized in the LP64 C interpreter.
    if (get_access_flags(method) & JVM_ACC_STATIC) {
      locals += BytesPerStackElement;
    }
    _kni_parameter_base = locals;
...
typedef struct JavaFrameDebug {
  address_word stack_bottom_pointer;
  address_word stored_int_value1;
  address_word stored_int_value2;
  address_word bcp_store;
  address_word locals_pointer;
  address_word cpool;
  address_word method;
  address_word caller_fp;
  address_word return_address;
} JavaFrameDebug;

JavaFrameDebug * jfp_debug() {
  return DERIVED(JavaFrameDebug*, g_jfp,
                 JavaFrame::stack_bottom_pointer_offset());
}
