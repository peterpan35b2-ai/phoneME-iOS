...
ReturnOop FileDescriptor::allocate(JVM_SINGLE_ARG_TRAPS) {
  if (!UseROM && Universe::before_main()) {
    ...
    const size_t object_size =
        align_allocation_size(valid_offset() + sizeof(jint));
    const int byte_length = (int)(object_size - Array::base_offset());
    GUARANTEE(byte_length >= 0, "dummy FileDescriptor size");
    return Universe::new_byte_array_raw(byte_length JVM_NO_CHECK_AT_BOTTOM);
  } else {
...
void FileDescriptor::set_handle(OsFile_Handle value) {
  address_word_field_put(handle_offset(), (address_word)value);
  set_valid(true);
}
...
void FileDescriptor::dispose() {
  if (valid()) {
    ...
    set_valid(false);
#ifdef AZZERT
    address_word_field_put(handle_offset(), (address_word)-1);
#endif
  }
}
