...
void JavaFrame::gc_epilogue(void) {
...
#endif
  {
    ...
    int     saved_bcp  = (int)(intptr_t)raw_bcp();
    ...
    address frame_bcp  = DERIVED(address, actual_bcp, flags);
...
void Frame::init(Thread *thread, address guessed_fp) {
  ...
  guessed_fp = (address)(((uintptr_t)guessed_fp) & ~(uintptr_t)0x03);
  limit      = (address)(((uintptr_t)limit)      & ~(uintptr_t)0x03);
  start      = guessed_fp;
