...
void ClassFileParser::gc_epilogue() {
  for (ClassFileParser* cfp = _head; cfp != NULL; cfp = cfp->previous()) {
    ...
    jubyte *base = (jubyte*)(cfp->_buffer->base_address());
    cfp->_bufptr = base + (intptr_t)cfp->_bufptr;
    cfp->_bufend = base + (intptr_t)cfp->_bufend;
  }
}
...
ReturnOop ClassFileParser::new_lazy_error_method(Method* method, 
                                                 ...
                                                 JVM_TRAPS)
{
  const int code_length = Method::native_method_code_size();
                              ...
                              const AccessFlags class_access_flags JVM_TRAPS)
{
  ...
  if (access_flags.is_native_or_abstract()) {
    ...
    code_length = Method::native_method_code_size();
  }
...
inline bool ClassFileParser::parse_class_0(ClassParserState *stack JVM_TRAPS) {
  ...
  // Some legacy/obfuscated MIDP jars mark an interface with ACC_INTERFACE
  // but omit the redundant ACC_ABSTRACT bit. Desktop JVMs accept these
  // class files and an interface is abstract by definition, so normalize the
  // flag instead of rejecting an otherwise usable MIDlet.
  if ((flags & JVM_ACC_INTERFACE) != 0 &&
      (flags & JVM_ACC_ABSTRACT) == 0) {
...
{
  {
    ...
    jvm_memcpy(new_cp_base_address, old_cp_base_address,
               cp_length * sizeof(OopDesc*));
  }
  {    
    ...
    for (; field < field_end; field += Field::NUMBER_OF_SLOTS) {
      ...
      if (field_size >= current_max) {
        ...
        if (basic_type == T_OBJECT || basic_type == T_ARRAY) {
          // Oop maps are encoded in 32-bit Java words. LP64 object
          // references may start on a 4-byte boundary.
          GUARANTEE((field_offset - prev_oop_offset) % BytesPerInt == 0,
                    "oop map offset must be Java-word aligned");
          size_t word_delta =
              (field_offset - prev_oop_offset) / BytesPerInt;
...
#if USE_DEBUG_PRINTING
  (void)st;
  tty->print_cr("ClassFileParser (StackObj) = %p", this);
