...
bool SourceROMWriter::execute0(JVM_SINGLE_ARG_TRAPS) {
  ...
  do {
    ...
    case STATE_WRITE_IMAGE:
      if (GenerateROMStructsOnly) {
        // Native 64-bit desktop builds keep classes.zip external. They still
        // need ROMStructs.h for KNI field access, but must not serialize the
        // 32-bit ROM image layout.
    ...
    case STATE_WRITE_STRUCT:
      if (!GenerateROMStructsOnly &&
          (GenerateROMStructs || GenerateROMStructsStatics)) {
        ROMStructsWriter rom_structs_writer;
        rom_structs_writer.write(this JVM_CHECK_0);
      }
      set_next_state();
...
#endif
    offset = by_ref ? (int)(intptr_t)object->obj() : _text_offset;
#if !USE_SEGMENTED_TEXT_BLOCK_WRITER
    if (offset == 0) {
...
#endif
    offset = by_ref ? (int)(intptr_t)object->obj() : _data_offset;
    break;
  case ROMWriter::HEAP_BLOCK:
    offset = _heap_offset;
    break;
