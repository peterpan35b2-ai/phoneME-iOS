...
inline bool ROM::heap_src_block_contains(address target) {
  const uintptr_t p = (uintptr_t)target -
                      (uintptr_t)&_rom_heap_block[0];
  const bool ret = p < (uintptr_t)_rom_heap_block_size;
  return ret;
}
...
OopDesc* ROM::raw_text_klass_of(const OopDesc* obj) {
  ...
  GUARANTEE((byte_offset < (uintptr_t)_rom_text_block_size),
            "must be in TEXT");
#endif
