...
public:
...
#else
    const uintptr_t offset = (uintptr_t)target -
                             (uintptr_t)&_rom_text_block[0];
    if (offset < (uintptr_t)_rom_text_block_size_fast) {
      return true;
    }
#endif  
    return false;
  ...
  static bool system_data_contains(const OopDesc* target) {
    const uintptr_t offset = (uintptr_t)target -
                             (uintptr_t)&_rom_data_block[0];
    if (offset < (uintptr_t)_rom_data_block_size_fast) {
      return true;
    } else {
      return false;
    }
