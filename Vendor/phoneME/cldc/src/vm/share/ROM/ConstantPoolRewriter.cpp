...
void ConstantPoolRewriter::free_unused_space(JVM_SINGLE_ARG_TRAPS) {
  ...
  if (unused_entries > 0) {
    ...
    // The filler object created by shrink_object must have native heap
    // alignment. On LP64 a 4-byte reduction cannot form a valid object.
    int unused_tags_entries = unused_entries & (~WordAlignmentMask);
