    ...
    for (SystemClassStream st; st.has_next_optimizable();) {
      ...
      if (nbytes > 0 && !dont_rename_fields_in_class(&klass)) {
        TypeArray::Raw reloc_info =
          Universe::new_int_array(nbytes / BytesPerInt JVM_CHECK);
    ...
    for( int i = 0; i < n; i++ ) {
      ...
      case JVM_CONSTANT_ResolvedStaticArrayFieldref: {
        ...
        if( reloc_info().not_null() ) {
          int idx =
              (offset - JavaClass::static_field_start()) / BytesPerInt;
          const int field_words =
              align_size_up(byte_size_for(type), BytesPerInt) / BytesPerInt;
          for (int word = 0; word < field_words; word++) {
            reloc_info().int_at_put(idx + word, 1);
          }
        }
      } break;
    ...
    for (int i = 0; i < n; i++) {
      ...
      switch (tag.value()) {
        ...
        case JVM_CONSTANT_ResolvedStaticArrayFieldref: {
          ...
          const int idx = (offset - static_field_start) / BytesPerInt;
          if (reloc_info().not_null()) {
            int dst = reloc_info().int_at(idx);
            int new_offset = (dst * BytesPerInt) + static_field_start;
    ...
    // (1) Move the fields
    {
      ...
      for (dst=src=0; src<reloc_info().length(); src++) {
        if (reloc_info().int_at(src) != 0) {
          if (dst != src) {
            int srcoffset = static_field_start + src * BytesPerInt;
            int dstoffset = static_field_start + dst * BytesPerInt;
      ...
      fields_shrink_size = (src - dst) * BytesPerInt;
    ...
    //     - Shrink the holder.
    {
      ...
      for (int i = 0; i < fields_len; i += Field::NUMBER_OF_SLOTS) {
        ...
        if (f.is_static()) {
          ...
          offset = f.offset();
          int idx = (offset - static_field_start) / BytesPerInt;
          int dst = reloc_info().int_at(idx);
      
          if ((dst != DEAD_FIELD) && (type == T_OBJECT || type == T_ARRAY)) {
            int new_offset = dst * BytesPerInt + static_field_start;
            // Oop maps use 32-bit Java-word deltas on all hosts.
            GUARANTEE((new_offset - prev_oop_offset) % BytesPerInt == 0,
                      "oop map offset alignment");
            int word_delta =
                (new_offset - prev_oop_offset) / BytesPerInt;
...
void ROMOptimizer::fix_one_field_table(const InstanceClass* klass,
  ...
  for (i = 0, j = 0; i < length; i+= Field::NUMBER_OF_SLOTS) {
    ...
    if (flags.is_static()) {
      ...
      const int new_offset = dst * BytesPerInt + static_field_start;
      GUARANTEE(new_offset <= offset, "sanity");
