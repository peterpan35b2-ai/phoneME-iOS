...
void ROMStructsWriter::write(ROMWriter *rom_writer JVM_TRAPS) {
  ...
  stream.print_cr("/* Java object fields are laid out in VM words without C ABI");
  stream.print_cr("   padding. LP64 C compilers would otherwise realign object");
  stream.print_cr("   references and make native field access use wrong offsets.");
  stream.print_cr("   Primitive arrays above retain natural ABI alignment so their");
  stream.print_cr("   data starts at ArrayDesc::header_size(). */");
  stream.print_cr("#if defined(JVM_64BIT) && JVM_64BIT");
  stream.print_cr("#pragma pack(push, 1)");
  stream.print_cr("#endif");
  stream.cr();
  ...
  stream.print_cr("#if defined(JVM_64BIT) && JVM_64BIT");
  stream.print_cr("#pragma pack(pop)");
  stream.print_cr("#endif");
  stream.cr();
...
void ROMStructsWriter::write_primitive_array_rom_struct(Stream *stream,
  ...
  stream->print_cr("#if defined(JVM_64BIT) && JVM_64BIT");
  stream->print_cr("    int ___header_pad;");
  stream->print_cr("#endif");
