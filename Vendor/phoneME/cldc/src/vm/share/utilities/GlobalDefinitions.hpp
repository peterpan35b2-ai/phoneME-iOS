...
// VM bitmaps are stored in 32-bit juint words even when native pointers are
// 64-bit. Keep their geometry independent from the native VM word size.
const int LogBitsPerWord =LogBitsPerInt;
...
const int BitsPerWord    = 1 << LogBitsPerWord;           //32 bitmap bits
...
// Heap and stack distances in phoneME are intentionally bounded to jint, but
// pointer arithmetic itself must happen at native width before narrowing.
#define DISTANCE(low, high) \
  ((jint)((intptr_t)(high) - (intptr_t)(low)))
#define DERIVED(type, ptr, offset) \
  ((type)((uintptr_t)(ptr) + (intptr_t)(offset)))
...
public:
  ...
  ArrayPointer(address native_addr) :
    _heap_array(NULL), _offset((intptr_t) native_addr) {}
...
enum BasicTypeByteSize {
  ...
  T_INT_byte_size     = 4,
  T_LONG_byte_size    = 8,
  T_OBJECT_byte_size  = BytesPerWord,
  T_ARRAY_byte_size   = BytesPerWord,
  T_SYMBOLIC_byte_size= BytesPerWord,
  T_VOID_byte_size    = 0
};
...
#if JVM_64BIT
  typedef OopDesc* SystemSymbolWord;
#else
  typedef int SystemSymbolWord;
#endif
#if ROMIZED_PRODUCT
  extern const SystemSymbolWord system_symbols[];
#else
  extern       SystemSymbolWord system_symbols[];
#endif
...
/*
 * The generated ARM fast_memclear stub is an ARM32 routine. Native Darwin
 * arm64 builds must use the libc-backed implementation or newly allocated
 * Java objects can retain stale field values after GC.
 */
