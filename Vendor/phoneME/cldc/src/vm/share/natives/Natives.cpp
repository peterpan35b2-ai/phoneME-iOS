...
// public static native boolean linkLegacyWtChain(Object root, Object value);
//
// Some Zelix-obfuscated MIDlets build transformation chains with thousands of
// calls to wt.a(wt). On the legacy C interpreter, invokeinterface/getfield
// quickening for these dynamically loaded classes can repeatedly dispatch on
// the old receiver. Traverse the package-private c:Lwt; link through KNI field
// access so every receiver update is GC-safe and architecture-independent.
jint Java_java_lang_System_linkLegacyWtChain() {
  jint linked = KNI_FALSE;
#ifndef PRODUCT
  int failure_reason = 0;
  int failure_step = -1;
  static int failure_reports = 0;
#endif

  KNI_StartHandles(4);
  KNI_DeclareHandle(current);
  KNI_DeclareHandle(value);
  KNI_DeclareHandle(clazz);
  KNI_DeclareHandle(next);

  KNI_GetParameterAsObject(1, current);
  KNI_GetParameterAsObject(2, value);

  if (!KNI_IsNullHandle(current) && !KNI_IsNullHandle(value)) {
    // The real Avatar graph is at most a few dozen nodes deep. The generous
    // limit also prevents a malformed/cyclic MIDlet graph from hanging VM.
    for (int i = 0; i < 65536; i++) {
      if (KNI_IsSameObject(current, value)) {
        linked = KNI_TRUE;
        break;
      }

      // Resolve the link field for every concrete node. Zelix may mix several
      // implementations of wt in one chain, and their field offsets differ.
      KNI_GetObjectClass(current, clazz);
      const jfieldID link_field = KNI_GetFieldID(clazz, "c", "Lwt;");
      if (link_field == 0) {
#ifndef PRODUCT
        failure_reason = 1;
        failure_step = i;
      ...
      KNI_GetObjectField(current, link_field, next);
      if (KNI_IsNullHandle(next)) {
        KNI_SetObjectField(current, link_field, value);
        linked = KNI_TRUE;
        break;
      }

      // Both identifiers refer to registered KNI handle slots. Swap the
      // identifiers rather than copying their internal representation; the
      // next field read will overwrite the old current slot safely.
      jobject swap = current;
      current = next;
      next = swap;
#ifndef PRODUCT
      failure_step = i;
...
#ifndef PRODUCT
  if (!linked && failure_reports < 4) {
    tty->print_cr("LINK_WT_FAIL reason=%d step=%d", failure_reason,
                  failure_step);
    failure_reports++;
  }
#endif
...
// CLDC omits Cloneable/CloneNotSupportedException, but many legacy MIDlets
// contain javac/obfuscator-generated array clone calls. Allocate array clones
// through the normal array allocator so the new object receives a fresh GC
// header; copying the complete source object would duplicate collector state.
ReturnOop Java_java_lang_Object_clone(JVM_SINGLE_ARG_TRAPS) {
  UsingFastOops fast_oops;
  JavaOop::Fast receiver = GET_PARAMETER_AS_OOP(0);

  if (receiver().is_int_array()) {
    TypeArray::Fast source = receiver.obj();
    const int length = source().length();
    TypeArray::Fast result =
        Universe::new_int_array(length JVM_CHECK_0);
    TypeArray::array_copy(&source, 0, &result, 0, length);
    return result.obj();
  }

  // Retain shallow-copy compatibility for ordinary objects. Avatar_LAPRO's
  // obfuscator only emits int[] clone calls, which take the safe path above.
  return ObjectHeap::clone(receiver.obj() JVM_NO_CHECK_AT_BOTTOM);
}
...
void Natives::register_natives_for(InstanceClass* c JVM_TRAPS) {
  ...
  for (const JvmNativesTable* table= jvm_natives_table; table->name!= NULL;
    ...
    if (class_name.equals(&name)) {
      ...
      /*
       * ROMized classes have this bit resolved by romgen. Classes loaded from
       * an external classes.zip are registered against the same static native
       * table at runtime, so mirror that romgen bookkeeping here. Otherwise
       * allocating any class with a private native finalize() incorrectly
       * throws UnsatisfiedLinkError even though the function was registered.
       */
      if (c->has_unresolved_finalizer()) {
        Method::Raw finalizer = c->lookup_finalizer();
        if (finalizer.not_null() && finalizer().is_native() &&
            finalizer().get_native_code() != NULL) {
          c->clear_has_unresolved_finalizer();
        }
      }
      return;
    }
