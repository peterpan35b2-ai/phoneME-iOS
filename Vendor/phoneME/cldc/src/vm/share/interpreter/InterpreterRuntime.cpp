...
#define STACK_LOCK_SIZE (StackLock::size() + BytesPerWord)
  ...
  Bytecodes::Code _rewrite_field_bytecode(Method &method,
    ...
    // Fast field bytecodes encode all 4-byte-or-larger field offsets in
    // Java 32-bit words. Object references may be 8 bytes on LP64, but the
    // class layout still permits them at 4-byte boundaries.
    if (::byte_size_for(type) >= BytesPerInt) {
      GUARANTEE(offset % BytesPerInt == 0, "Must be Java-word aligned");
      offset = offset >> LogBytesPerInt;
    }
