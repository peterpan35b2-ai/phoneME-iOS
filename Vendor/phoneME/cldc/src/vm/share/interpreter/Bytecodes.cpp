...
#if JVM_64BIT
  def(fast_invokenative         , 16, "bc__jjjj________", 0, ""   , Exceptions | NoFallThru | NoPatching),
#else
...
int Bytecodes::wide_length_for(const Method* method, int bci, Code code) {
  ...
  case _tableswitch:{
    ...
    int lo = method->get_java_switch_int(a_bci + switch_word_size);
    int hi = method->get_java_switch_int(a_bci + 2 * switch_word_size);
    return a_bci + (3 + hi - lo + 1) * switch_word_size - bci;
  }
  case _lookupswitch: {
    const int switch_word_size = sizeof(jint);
    int a_bci = align_size_up(bci + 1, switch_word_size);
    int npairs = method->get_java_switch_int(a_bci + switch_word_size);
    return a_bci + (2 + 2 * npairs) * switch_word_size - bci;
  }
#if !ENABLE_CPU_VARIANT
