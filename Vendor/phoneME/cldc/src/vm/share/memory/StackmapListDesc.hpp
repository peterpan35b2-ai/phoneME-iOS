...
class StackmapListDesc: public OopDesc {
 private:
  ...
  static size_t allocation_size(int length) { 
    // Each stackmap entry contains either a compact 32-bit value or a
    // native object pointer. Keep the carrier pointer-sized on LP64.
    return align_allocation_size(header_size() +
                                 length * sizeof(address_word));
  }
 ...
 private:
  ...
  address_word* stackmap_entries_base() {
    return (address_word*)((address)this + header_size());
  }
