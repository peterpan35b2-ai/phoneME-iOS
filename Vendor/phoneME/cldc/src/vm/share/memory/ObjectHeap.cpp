...
#define WRITE_BARRIER_OOPS_LOOP_BEGIN(start, end, p) \
  const size_t wb_start_index = oop_boundary_index(start); \
  const size_t wb_end_index = oop_boundary_index(end); \
  for (size_t wb_index = wb_start_index; wb_index < wb_end_index; \
       wb_index++) { \
    OopDesc** p = DERIVED(OopDesc**, _heap_start, \
                          wb_index * BytesPerInt); \
    if (test_bit_for(p)) { \
      if (TraceGC) { \
        TTY_TRACE_CR(("TraceGC: %p write barrier entry", p)); \
      }
...
OopDesc* ObjectHeap::allocate(size_t size JVM_TRAPS) {
  ...
  GUARANTEE((uintptr_t)_inline_allocation_top % BytesPerWord == 0,
            "Sanity");
  PERFORMANCE_COUNTER_INCREMENT(num_of_c_alloc_objs, 1);
...
size_t ObjectHeap::update_slices_size(size_t object_heap_size) {
  ...
  size_t near_bits = 0;
  size_t s;
  for (s = (object_heap_size - 1) >> LogBytesPerWord; s != 0; s >>= 1) {
    near_bits++;
  }
...
bool ObjectHeap::adjust_heap_size(size_t target_heap_size) {
  ...
  const size_t bitvector_size =
      target_heap_size / (BytesPerInt * BitsPerByte);
  ...
  if (is_new_heap) {
    ...
    if (_heap_min == _heap_capacity) {
      ...
      max_bitv = _heap_capacity / (BytesPerInt * BitsPerByte)
                + max_slices_size;
  ...
  must_be_aligned((size_t)_heap_chunk);
  must_be_aligned((size_t)_bitv_chunk);
  ...
  // Use heap-relative bitmap indexes. The old implementation biased this
  // pointer by the absolute 32-bit heap address, which is invalid on LP64.
  _bitvector_base = _bitvector_start;
...
inline OopDesc** ObjectHeap::mark_forward_pointers( void ) {
  ...
  while (p < end_scan && test_bit_for(p, bitvector_base)) {
    ...
    if (instance_size > 0) {
      ...
      address base_addr = (address)p;
      for (;;) {
        const jint entry = jint(*map++);
        if (entry > 0) {
          base_addr += entry * BytesPerInt;
          OopDesc** base = (OopDesc**)base_addr;
          ...
          base_addr += (OopMapEscape - 1) * BytesPerInt;
        }
      }
    } else {
...
inline OopDesc* ObjectHeap::rom_oop_from_offset(size_t offset,
                                                const QuickVars& qv) {
  if ((offset & 0x1) == 0) {
    return DERIVED(OopDesc*, qv.rom_text_start, offset);
  } else {
    offset --;
    return DERIVED(OopDesc*, qv.rom_data_start, offset);
  }
}
...
inline OopDesc* ObjectHeap::rom_oop_from_offset(size_t offset) {
  if ((offset & 0x1) == 0) {
    ...
    return DERIVED(OopDesc*, ROM::min_text_seg_addr(), offset);
#else
    return DERIVED(OopDesc*, _rom_text_block, offset);
#endif
  } else {
    offset --;
    return DERIVED(OopDesc*, _rom_data_block, offset);
  }
}
...
inline void ObjectHeap::compute_new_object_locations() {
  ...
  while (p < inline_allocation_top) {
    ...
    if (test_bit_for(p, bitvector_base)) {
      ...
      OopDesc** bitvector_aligned =
          ObjectHeap::get_aligned_for_bitvectorword(bitvector_word_ptr);
      const int trash_bits =
          DISTANCE(bitvector_aligned, p) / BytesPerInt;
      GUARANTEE(0 <= trash_bits && trash_bits <= 31, "Sanity");
      ...
      p = ObjectHeap::get_aligned_for_bitvectorword(bitvector_word_ptr);
      // Find first bit set in word (if any). Each bit represents one
      // 32-bit Java word, not one native OopDesc* slot.
      if ((bitword & 0xFFFF) == 0) {
        bitword >>= 16; p = DERIVED(OopDesc**, p, 16 * BytesPerInt);
      }
      if ((bitword &   0xFF) == 0) {
        bitword >>=  8; p = DERIVED(OopDesc**, p,  8 * BytesPerInt);
      }
      if ((bitword &    0xF) == 0) {
        bitword >>=  4; p = DERIVED(OopDesc**, p,  4 * BytesPerInt);
      }
      if ((bitword &    0x3) == 0) {
        bitword >>=  2; p = DERIVED(OopDesc**, p,  2 * BytesPerInt);
      }
      if ((bitword &    0x1) == 0) {
        bitword >>=  1; p = DERIVED(OopDesc**, p,      BytesPerInt);
      }
  ...
  while (this_stack != NULL) {
    ...
    if ((OopDesc**)this_stack < _end_fixed_objects) {
      ...
      *previous_stack_addr = next_stack;
      ...
      const int delta = DISTANCE(this_stack, destination);

      if (TraceGC) {
        TTY_TRACE_CR(("TraceGC: Stack %p relocated => %p",
...
void ObjectHeap::write_barrier_oops_update_moving_object_interior_pointers(
  ...
  WRITE_BARRIER_OOPS_LOOP_BEGIN(start, end, p);
  {
    ...
    if (instance_size > 0) {
      ...
      address interior_addr = (address)p;
      OopDesc** lower_limit = qv.compaction_start;
      OopDesc** upper_limit = qv.collection_area_end;
      while (true) {
        jint entry = (jint)(*map++);
        if (entry > 0) {
          interior_addr += entry * BytesPerInt;
          OopDesc** interiorp = (OopDesc**)interior_addr;
          ...
          GUARANTEE((entry & 0xff) == OopMapEscape, "sanity")
          interior_addr += (OopMapEscape - 1) * BytesPerInt;
        }
      }
      ...
      if (instance_size == InstanceSize::size_compiled_method) {
        const int delta = DISTANCE(p, destination);
...
OopDesc* ObjectHeap::decode_near(OopDesc* obj, const QuickVars& qv ) {
  ...
  const juint encoded_near = (juint)(uintptr_t)klass;
  const size_t near_offset = encoded_near & qv.near_mask;
  OopDesc* n;
#if ENABLE_HEAP_NEARS_IN_HEAP
    GUARANTEE((jint)encoded_near >= 0, "optimization check");
    n = (OopDesc*)((OopDesc**) qv.heap_start + near_offset);
    GUARANTEE(contains(n), "must be in heap");
#else
  if ((jint)encoded_near >= 0) {
...
inline OopDesc* ObjectHeap::decode_near(OopDesc* obj, OopDesc **heap_start, 
  ...
  const OopDesc* klass = obj->klass();
  const juint encoded_near = (juint)(uintptr_t)klass;
  const size_t near_offset = encoded_near & near_mask;
  OopDesc* n;
#if ENABLE_HEAP_NEARS_IN_HEAP
    GUARANTEE((jint)encoded_near >= 0, "optimization check");
...
#if JVM_64BIT
  for (address p = (address)start; p < (address)end; p += BytesPerInt) {
    clear_bit_for((OopDesc**)p);
  }
  return;
#else
...
#ifndef PRODUCT
    tty->print_cr("OOM_DIAG alloc=%d used=%d free=%d limit=%d reserve=%d",
                  (int)alloc_size, used, (int)free_memory(), limit, reserve);
    Thread::current()->trace_stack(tty);
#endif
...
inline void* ObjectHeap::set_heap_limit( void* new_heap_limit ) {
  ...
  if( !in_use && _heap_min == _heap_capacity ) {
    ...
    new_heap_limit = (void*)((uintptr_t)new_heap_limit &
...
void ObjectHeap::print(Stream* st) {
  st->print_cr("Object heap       [%p,%p), %d bytes",
                                   _heap_start, _heap_top,
                                   DISTANCE(_heap_start, _heap_top));
  st->print_cr("total/free bytes    [%d,%d]", total_memory(), free_memory());
  st->print_cr("- Collection area   [%p,%p, %p)",
    _collection_area_start, _inline_allocation_top, _inline_allocation_end);
  st->print_cr("- Compiler area     [%p,%p, %p)",
    _compiler_area_start, _compiler_area_top, compiler_area_end());
  st->print_cr("- Large object area [%p,%p, %p)",
    LargeObject::start(), LargeObject::bottom(), LargeObject::end());
  st->print_cr("Min marking stack   [%p,%p)", _heap_top, _bitvector_start);
  st->print_cr("Bit vector          [%p,%p), %d bytes",
                                  _bitvector_start, _slices_start,
                                  DISTANCE(_bitvector_start, _slices_start));
  st->print_cr("Slices start      %p",          _slices_start);
  ...
  if (Verbose) {
    for (size_t i = 0; i < _nof_slices; i++) {
      st->print_cr("- Slice[%4d] %p", i, _slices_start[i]);
    }
  }
  print_task_usage(st);
}
