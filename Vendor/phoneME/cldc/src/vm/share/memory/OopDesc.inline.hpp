...
void OopDesc::map_oops_do(jubyte* map, void do_oop(OopDesc**)) {
  ...
  for (;;) {
    ...
    if (entry > 0) {
      base += entry * BytesPerInt;
      do_oop((OopDesc**)base);
    } else if (entry == 0) {
      break;
    } else {
      GUARANTEE((entry & 0xff) == OopMapEscape, "sanity")
      base += (OopMapEscape - 1) * BytesPerInt;
    }
  }
}

void OopDesc::map_native_oops_do(jubyte* map, void do_oop(OopDesc**)) {
  // Native VM metadata oop maps are generated from C++ structure offsets
  // and therefore use host VM words rather than 32-bit Java words.
  address base = (address)this;
  for (;;) {
    const jint entry = (jint)(*((jbyte*)map)); map++;
    if (entry > 0) {
      base += entry * BytesPerWord;
      do_oop((OopDesc**)base);
    } else if (entry == 0) {
      break;
    } else {
      GUARANTEE((entry & 0xff) == OopMapEscape, "sanity")
      base += (OopMapEscape - 1) * BytesPerWord;
    }
  }
}
