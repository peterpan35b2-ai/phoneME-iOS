...
private:  
 ...
 private:
  ...
  static size_t oop_index (OopDesc** p) {
    GUARANTEE(contains(p), "Should be in object heap");
    return ((uintptr_t)p - (uintptr_t)_heap_start) >> LogBytesPerInt;
  }

  static size_t oop_boundary_index(OopDesc** p) {
    // The write-barrier map has one bit per 32-bit Java word, not per
    // native pointer. LP64 object fields are allowed on a 4-byte boundary.
    GUARANTEE((uintptr_t)p >= (uintptr_t)_heap_start &&
              (uintptr_t)p <= (uintptr_t)_heap_top,
              "Should be an object heap boundary");
    return ((uintptr_t)p - (uintptr_t)_heap_start) >> LogBytesPerInt;
  }

  static size_t bitvector_word_index  (const size_t i) {
    return i >> LogBitsPerWord;
  }

  static unsigned bitvector_bit_index   (const size_t i) {
    return (unsigned)(i & (BitsPerWord-1));
  }

  static juint bitvector_bit_mask    (const size_t i) {
    return ((juint)1) << bitvector_bit_index(i);
  }

  static juint& bitvector_word( const size_t i,
                                address bitvector_base = _bitvector_base) {
    return ((juint*)bitvector_base)[ bitvector_word_index( i ) ];
  }
...
private:
  ...
  static void must_be_aligned( const size_t n ) {
    (void)n;
...
public:
  static juint* get_bitvectorword_for_aligned(OopDesc** p) {
    ...
    return ((juint*)_bitvector_base) +
           bitvector_word_index(oop_boundary_index(p));
  }
private:
  ...
  static OopDesc** get_aligned_for_bitvectorword(juint *ptr) {
    const size_t bitmap_words = (size_t)(ptr - (juint*)_bitvector_base);
    return DERIVED(OopDesc**, _heap_start,
                   bitmap_words * BitsPerWord * BytesPerInt);
  }
  ...
  enum {
    alignment = BitsPerWord * BytesPerInt
  };
 ...
 */
inline juint* word_for(size_t offset, juint* map) {
  return map + (offset/BitsPerWord);
}

inline void bit_clear(size_t offset, juint* map) {
  juint* w = word_for(offset, map);
  const unsigned pos = (unsigned)(offset % BitsPerWord);
  *w &= ~(((juint)1) << pos);
}

inline void bit_set(size_t offset, juint* map) {
  juint* w = word_for(offset, map);
  const unsigned pos = (unsigned)(offset % BitsPerWord);
  *w |= ((juint)1) << pos;
}

inline bool is_bit_set(size_t offset, juint* map) {
  return ((*word_for(offset, map)) & (((juint)1) << (offset % BitsPerWord))) != 0;
}

inline void ObjectHeap::set_bit_for(OopDesc** p, address bitvector_base) {
  const size_t i = oop_index(p);
...
inline bool ObjectHeap::test_and_set_bit_for(OopDesc** p,
  ...
  const size_t i = oop_index(p);
  const juint bitword = bitvector_word( i, bitvector_base );
  const juint mask = bitvector_bit_mask( i );
...
inline void ObjectHeap::set_bit_range(OopDesc** start, int len) {
  size_t t = oop_index(start);
  juint* const bitvector_base = (juint*)_bitvector_base;
  const size_t slot_step = BytesPerWord / BytesPerInt;
  for (int i = 0; i < len; i++, t += slot_step) {
