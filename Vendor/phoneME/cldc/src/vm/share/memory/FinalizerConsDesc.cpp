...
void FinalizerConsDesc::run_finalizer( void ) {
  ...
  
  {
    ...
    // the receiver. Keep a real local slot alive across the native call;
    // taking the address of the Raw handle field can be optimized away on
    // LP64 release builds because the compiler does not see that KNI reads it.
    receiver_slot = r.obj();
    _kni_parameter_base = (unsigned char *) &receiver_slot;
