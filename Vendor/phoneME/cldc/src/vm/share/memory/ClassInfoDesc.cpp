...
void ClassInfoDesc::variable_oops_do(void do_oop(OopDesc**)) {
  ...
  // itable. Each interface descriptor is two 32-bit integers even on
  // LP64; only the method entries that follow are native oop-sized slots.
  address itable_bytes = (address)vtable;
  itable_bytes += _itable_length * (sizeof(jint) + sizeof(jint));
  OopDesc** itable = (OopDesc**)itable_bytes;
