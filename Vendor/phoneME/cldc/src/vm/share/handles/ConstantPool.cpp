...
ReturnOop ConstantPool::name_of_klass_at(int index JVM_TRAPS) {
  cp_check_0(is_within_bounds(index));
  const jubyte tag = tag_value_at(index);
  const int offset = offset_from_index(index);
  const address_word raw = address_word_field(offset);
  ReturnOop result;

#if JVM_64BIT && ENABLE_ROM_GENERATOR
  if ((raw & (address_word)0xffff000000000000ULL) != 0) {
    jvm_fprintf(stderr,
                "Suspicious CP class slot: cp=%p index=%d tag=%d offset=%d slot=%p raw=%p\n",
                obj(), index, tag, offset,
                obj()->address_word_field_addr(offset),
                (void*)raw);
    jvm_fflush(stderr);
  }
#endif

  if (ConstantTag::is_unresolved_klass(tag)) {
    result = unchecked_unresolved_klass_at(index);
  } else {
...
#if JVM_64BIT && ENABLE_ROM_GENERATOR
  if (((address_word)result & (address_word)0xffff000000000000ULL) != 0) {
    jvm_fprintf(stderr,
                "Invalid CP class name: cp=%p index=%d tag=%d offset=%d raw=%p result=%p\n",
                obj(), index, tag, offset, (void*)raw, result);
    jvm_fflush(stderr);
  }
#endif
  return result;
                               ...
                               InstanceClass* static_receiver_class JVM_TRAPS)
{
  ...
  if (m.is_null()) {
#ifndef PRODUCT
    tty->print("Missing method during resolution: ");
    static_receiver_class->print_value_on(tty);
    tty->print(".");
