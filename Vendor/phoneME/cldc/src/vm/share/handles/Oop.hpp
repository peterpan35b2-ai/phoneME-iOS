...
public:
  ...
  /// Native pointer-width field access operations.
  address_word address_word_field(int offset) const {
    return *obj()->address_word_field_addr(offset);
  }
  void address_word_field_put(int offset, address_word value) {
    GUARANTEE(!oop_check_barrier(obj()->obj_field_addr(offset)), "Write barrier bit set");
    *obj()->address_word_field_addr(offset) = value;
  }
...
#define __HANDLE_DEFINITION__(this_class, super_class) \
  ...
  public:                                        \
    ...
    /* Modern optimizing compilers may reuse a trivial local Fast handle's */ \
    /* stack slot before UsingFastOops performs its debug verification. */ \
    /* Pop explicitly in assertion builds so the chain never references */ \
    /* an object whose lifetime has ended. Product builds retain bulk pop. */ \
    AZZERT_ONLY(~Fast() { pop_handle(); })       \
  }
