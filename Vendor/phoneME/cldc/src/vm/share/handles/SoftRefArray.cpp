...
ReturnOop SoftRefArray::create(int length JVM_TRAPS) {
  ...
  // Reserve the first element for null references. Soft-reference slots
  // contain tagged native heap pointers, so use 64-bit elements on LP64.
#if JVM_64BIT
  Array::Raw array = Universe::new_long_array(length + 1 JVM_CHECK_0);
#else
  Array::Raw array = Universe::new_int_array(length + 1 JVM_CHECK_0);
#endif
  address base = array().base_address();
  memset(base + sizeof(jobject), -1, length * sizeof(jobject));
  return array.obj();
}
