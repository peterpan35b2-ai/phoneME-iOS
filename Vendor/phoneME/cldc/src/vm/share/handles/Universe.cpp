...
ReturnOop Universe::new_instance_class(int vtable_length,
  ...
  // The embedded oop map starts immediately after the native descriptor and
  // embedded static fields. Do not use allocation_size(..., 0, 0) here: on
  // LP64 its final 8-byte alignment can skip over the bytes reserved for a
  // small oop map and overwrite the next heap object.
  size_t oop_map_start = sizeof(InstanceClassDesc);
#if !ENABLE_ISOLATES
  oop_map_start += static_field_size;
#endif
  result().set_embedded_oop_map_start(oop_map_start);
  result().set_class_info(&info);
