...
#endif
  const address_word n = address_word_field(stackmaps_offset());
  if ((n & 0x01) != 0) {
    ...
    sm().entry_word_put(0, (address_word)(n >> 1));
    ...
    for (bci = 0; bcptr < bcend;) {
      ...
      if (code >= Bytecodes::_breakpoint ||!Bytecodes::is_defined(code)) {
        ...
        case Bytecodes::_tableswitch:      // 0xaa
          {
            const int switch_word_size = sizeof(jint);
            int a_bci = align_size_up(bci + 1, switch_word_size);
            // Bounds check: should be able to read at least 2 Java words
            // starting with a_bci.
            if (a_bci + 2 * switch_word_size > codesize) {
              goto error;
            }
            ...
            int fields;
            if (code == Bytecodes::_tableswitch) {
              // Bounds check: should be able to read at least 3 Java words
              // starting with a_bci.
              if (a_bci + 3 * switch_word_size > codesize) {
                goto error;
              }
              int raw_lo = get_native_aligned_int(a_bci + switch_word_size);
              int raw_hi = get_native_aligned_int(a_bci + 2 * switch_word_size);
              ...
              int raw_npairs =
                  get_native_aligned_int(a_bci + switch_word_size);
              if (Bytes::is_Java_byte_ordering_different()) {
            ...
            int end = a_bci + fields * switch_word_size;
            ...
            if (Bytes::is_Java_byte_ordering_different()
              ...
              for (int offset = a_bci; offset < end;
                   offset += switch_word_size) {
