...
void TypeSymbol::ParseStream::gc_prologue() {
  for (ParseStream*p = _list; p; p=p->_next) {
    if (p->_heap_input != NULL) {
      ...
      char* const base = (char*)p->_heap_input->base_address();
      const intptr_t pos = p->_current_ptr - base;
      p->_current_ptr = (char*)pos;
...
void TypeSymbol::ParseStream::gc_epilogue() {
  for (ParseStream*p = _list; p; p=p->_next) {
    if (p->_heap_input != NULL) {
      ...
      const intptr_t pos = (intptr_t)p->_current_ptr;
