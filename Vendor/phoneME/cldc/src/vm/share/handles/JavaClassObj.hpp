...
class JavaClassObj : public Instance {
 public:
  ...
  static int status_offset() {
    return java_class_offset() + BytesPerWord;
  }
  static int thread_offset() {
    // java.lang.Class declares: Object vmClass, int status, Thread thread.
    // LP64 references occupy two Java words, but fields retain declaration
    // order and may therefore start on a 4-byte boundary.
    return status_offset() + BytesPerInt;
  }

#if USE_REFLECTION
  static int type_symbol_offset() {
    return thread_offset() + BytesPerWord;
  }
