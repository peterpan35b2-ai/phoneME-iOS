...
class StackmapList: public Oop {
 ...
 public:
  ...
  static int entry_stat_offset(int index) {
    return header_size() + sizeof(address_word) * index;
  }

  address_word entry_word(int index) const {
    return address_word_field(entry_stat_offset(index));
  }

  void entry_word_put(int index, address_word value) {
    address_word_field_put(entry_stat_offset(index), value);
  }

 private:
  inline bool is_valid(int index) {
    return entry_word(index) != 0;
  }
 ...
 public:
  ...
  void set_short_map(int index, unsigned short bitmap) {
    ...
    entry &= 0xFFFF;
    entry |= (bitmap << 16);
    entry_word_put(index, (address_word)entry);
