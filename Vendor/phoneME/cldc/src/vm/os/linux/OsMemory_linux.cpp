...
static inline address anon_mmap(address addr, size_t size) {
  ...
  // Apple Silicon enforces W^X. The C-interpreter heap never contains
  // executable code, so requesting RWX makes mmap fail with EPERM.
  const int protection = PROT_READ | PROT_WRITE;
#else
  const int protection = PROT_EXEC | PROT_WRITE | PROT_READ;
#endif
  return (address)jvm_mmap(addr, size, protection,
                                ...
                                size_t max_size, size_t alignment)
{
  ...
  GUARANTEE((uintptr_t)chunk % alignment == 0, "must be aligned");
  GUARANTEE((uintptr_t)chunk % SysPageSize == 0, "must be page aligned");
