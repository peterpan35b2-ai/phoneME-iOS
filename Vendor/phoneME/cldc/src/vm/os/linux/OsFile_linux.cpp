...
static char* osfile_path_to_utf8(const JvmPathChar* path) {
  const JvmPathChar* cursor = path;
  size_t byte_count = 0;

  while (*cursor != 0) {
    juint codepoint = *cursor++;
    if (codepoint >= 0xd800 && codepoint <= 0xdbff &&
        *cursor >= 0xdc00 && *cursor <= 0xdfff) {
      codepoint = 0x10000 + ((codepoint - 0xd800) << 10) +
                  (*cursor++ - 0xdc00);
    }
    byte_count += codepoint < 0x80 ? 1 :
                  codepoint < 0x800 ? 2 :
                  codepoint < 0x10000 ? 3 : 4;
  }

  char* result = (char*)jvm_malloc(byte_count + 1);
  if (result == NULL) {
    return NULL;
  }
...
static void osfile_release_utf8(char* path) {
  jvm_free(path);
}
#else
static char* osfile_path_to_utf8(const JvmPathChar* path) {
...
OsFile_Handle OsFile_open(const JvmPathChar *filename, const char *mode) {
  char* utf8_filename = osfile_path_to_utf8(filename);
  if (utf8_filename == NULL) {
    return NULL;
  }
  OsFile_Handle result = (OsFile_Handle)jvm_fopen(utf8_filename, mode);
  osfile_release_utf8(utf8_filename);
  return result;
}
...
bool OsFile_exists(const JvmPathChar *name) {
  char* utf8_name = osfile_path_to_utf8(name);
  if (utf8_name == NULL) {
    return false;
  }

  struct stat buf;
  bool result = jvm_stat(utf8_name, &buf) == 0 && S_ISREG(buf.st_mode);
  osfile_release_utf8(utf8_name);
  return result;
}
...
bool OsFile_rename(const JvmPathChar *from, const JvmPathChar *to) {
  char* utf8_from = osfile_path_to_utf8(from);
  char* utf8_to = osfile_path_to_utf8(to);
  if (utf8_from == NULL || utf8_to == NULL) {
    osfile_release_utf8(utf8_from);
    osfile_release_utf8(utf8_to);
    return false;
  }

  bool result = jvm_rename(utf8_from, utf8_to) == 0;
  osfile_release_utf8(utf8_from);
  osfile_release_utf8(utf8_to);
  return result;
}
...
int OsFile_remove(const JvmPathChar *filename) {
  ...
  int result = jvm_remove(utf8_filename);
  osfile_release_utf8(utf8_filename);
  return result;
}
#endif // !ENABLE_PCSL
