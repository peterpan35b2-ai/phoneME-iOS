...
TEST_ROOT="$(phoneme_make_isolated_root \
  "$CORE_ROOT" \
  "cldc-library-host" \
  "${PHONEME_CLDC_TEST_ROOT:-}")"
phoneme_register_cleanup "$TEST_ROOT"
STUB_CLASSES="$TEST_ROOT/stubs"
FIXTURE_CLASSES="$TEST_ROOT/classes"
FIXTURE_JAR="$TEST_ROOT/cldc-library.jar"
TEST_BINARY="$TEST_ROOT/CldcLibraryTests"
...
[[ -n "$JAVAC" && -n "$JAR" ]] || {
  echo "javac and jar are required for CLDC library tests." >&2
  exit 1
}
...
while IFS= read -r source; do
  ...
  [[ "$source" == */network/PosixNetworkAdapter.cpp ]] && continue
  SOURCES+=("$source")
...
"$CXX" \
  ...
  "$CORE_ROOT/Tests/CldcLibraryTests.cpp" \
  "${SOURCES[@]}" \
  -lz \
  -framework CoreText \
