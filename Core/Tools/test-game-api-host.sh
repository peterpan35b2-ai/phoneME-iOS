...
TEST_ROOT="$(phoneme_make_isolated_root "$CORE_ROOT" "game-api-host-tests" "${PHONEME_GAME_API_TEST_ROOT:-}")"
phoneme_register_cleanup "$TEST_ROOT"
STUB_CLASSES="$TEST_ROOT/compile-stubs"
FIXTURE_CLASSES="$TEST_ROOT/fixture-classes"
FIXTURE_JAR="$TEST_ROOT/game-api-fixture.jar"
TEST_BINARY="$TEST_ROOT/GameApiVmTests"
...
"$JAVAC" -source 8 -target 8 -Xlint:-options \
  ...
  "$CORE_ROOT/Tests/fixtures/GameApiOps.java"
"$JAR" cf "$FIXTURE_JAR" -C "$FIXTURE_CLASSES" .

SOURCES=()
while IFS= read -r source; do
  case "$source" in
    */api/CAPI.cpp|*/vm/CoreNatives.cpp|*/network/PosixNetworkAdapter.cpp) continue ;;
  esac
  SOURCES+=("$source")
...
"$CXX" -std=c++23 -isysroot "$SDK_ROOT" \
  ...
  "$CORE_ROOT/Tests/GameApiVmTests.cpp" "${SOURCES[@]}" \
