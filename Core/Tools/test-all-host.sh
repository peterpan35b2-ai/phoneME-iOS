...
if [[ "$#" -gt 0 ]]; then
  ...
  MODULES=(host builtin-registry graphics graphics-vm push canvas-graphics game-api filesystem network-adapter security c-api rms scheduler suite-installer lcdui-extended coverage-inventory)
fi
